...
const SPREADS = [
  ...
  },
  {
    left: {
      ...
      items: [
        { name: 'Nem cuốn tươi',      price: '45.000đ', desc: 'Tôm tươi, thịt luộc, rau sống, bánh tráng', img: IMGS.nemcuon },
    ...
    right: {
      ...
      items: [
        { name: 'Phở bò truyền thống', price: '85.000đ', desc: 'Nước dùng hầm 12 tiếng, thịt tái chín', img: IMGS.pho },
  ...
  },
  {
    left: {
      ...
      items: [
        { name: 'Chả cá Lã Vọng', price: '145.000đ', desc: 'Cá lăng tươi, nghệ, thì là, mắm tôm',       img: IMGS.cakho },
    ...
    right: {
      ...
      items: [
        { name: 'Cà phê trứng',   price: '55.000đ', desc: 'Cà phê Robusta, lòng đỏ trứng gà, sữa đặc', img: IMGS.coffee },
      ...
      <Container>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <button
            ...
            style={{ ...btnStyle(!canPrev), position: 'absolute', left: -60, top: '50%', transform: 'translateY(-50%)', zIndex: 20 }}
          ...
          <button
            ...
            style={{ ...btnStyle(!canNext), position: 'absolute', right: -60, top: '50%', transform: 'translateY(-50%)', zIndex: 20 }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24, marginTop: 36 }}>
          <div style={{ display: 'flex', gap: 28 }}>
            <button
              onClick={() => flip('backward')} disabled={!canPrev}
              style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', cursor: canPrev ? 'pointer' : 'not-allowed', color: canPrev ? 'rgba(253,246,227,.55)' : 'rgba(201,164,90,.2)', fontFamily: 'var(--font-sans)', fontSize: 13 }}
            >
              <ChevronLeft size={16} /> {t.prev}
            </button>
            <button
              onClick={() => flip('forward')} disabled={!canNext}
              style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', cursor: canNext ? 'pointer' : 'not-allowed', color: canNext ? 'rgba(253,246,227,.55)' : 'rgba(201,164,90,.2)', fontFamily: 'var(--font-sans)', fontSize: 13 }}
            >
...
function Page({ page, side }) {
  return page.type === 'cover' ? <CoverPage variant={page.variant} /> : <ContentPage page={page} side={side} />
}
...
function CoverPage({ variant }) {
  ...
  return (
    <div style={{
      ...
      {/* Corner ornaments (right page only) */}
      {!isLeft && (
        <>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        {isLeft ? (
          <>
            ...
            <p style={{ fontFamily: 'var(--font-serif)', color: 'rgba(253,246,227,.4)', fontSize: 'clamp(.85rem,1.8vw,1.05rem)', fontStyle: 'italic', lineHeight: 1.85 }}>
          ...
          <>
            ...
            <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(1.8rem,4.5vw,3.5rem)', fontWeight: 700, color: '#fdf6e3', lineHeight: 1, marginBottom: 8 }}>
              Bếp Hà Thành
            </h2>
            <div style={{ width: 56, height: 1, background: 'rgba(201,164,90,.5)', margin: '16px auto' }} />
            <p style={{ fontFamily: 'var(--font-serif)', color: 'rgba(201,164,90,.8)', fontSize: 'clamp(.9rem,1.8vw,1.2rem)', fontStyle: 'italic' }}>Thực Đơn</p>
...
/* ── Content page ─────────────────────────────── */
function ContentPage({ page, side }) {
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', zIndex: 3, height: '100%', display: 'flex', flexDirection: 'column', padding: 'clamp(14px, 2.5vw, 28px)' }}>
        {/* Header */}
        <div style={{ marginBottom: 14, paddingBottom: 10, borderBottom: '1px solid rgba(155,27,46,.2)' }}>
          <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(1.1rem,2.5vw,1.7rem)', fontWeight: 700, color: '#9b1b2e', lineHeight: 1, marginBottom: 3 }}>
            {page.category}
          </h3>
        ...
        {/* Dishes */}
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(8px, 1.4vw, 14px)' }}>
          {page.items.map((item) => (
            <div key={item.name} style={{ display: 'flex', gap: 'clamp(7px, 1.2vw, 12px)', alignItems: 'flex-start' }}>
              <div style={{
                width: 'clamp(48px, 6.5vw, 72px)', height: 'clamp(48px, 6.5vw, 72px)', flexShrink: 0,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 6, marginBottom: 2 }}>
                  <p style={{ fontFamily: 'var(--font-serif)', fontWeight: 700, fontSize: 'clamp(.68rem,1.3vw,.88rem)', color: '#1c0a00', lineHeight: 1.3 }}>
                    {item.name}
                  </p>
                  <span style={{ fontFamily: 'var(--font-serif)', fontWeight: 700, fontSize: 'clamp(.65rem,1.2vw,.84rem)', color: '#9b1b2e', flexShrink: 0 }}>
                    {item.price}
                  </span>
                </div>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: 'clamp(9px, 1vw, 11px)', color: 'rgba(28,10,0,.45)', lineHeight: 1.45 }}>
                  {item.desc}
                </p>
              </div>
            </div>
